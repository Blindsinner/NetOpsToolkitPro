...
from PySide6.QtWidgets import (
    ...
    QSplitter, QPlainTextEdit, QTreeWidget, QTreeWidgetItem, QLabel
)
...
class DeviceDialog(QDialog):
    def __init__(self, cred_manager: CredentialsManager, device_data=None, parent=None):
        super().__init__(parent)
    ...
    def test_connection(self):
        ...
        self.parent().task_manager.create_task(do_test())
...
class ConfigManagerDialog(QDialog):
    def __init__(self, task_manager: TaskManager, parent=None):
        ...
        self.setWindowTitle("Configuration Management")
    ...
    def run_compliance_audit(self):
        if not self.devices:
            QMessageBox.warning(self, "No Devices", "Please add devices to the inventory before running an audit.")
            return
    ...
    def _get_decrypted_device(self, device_info):
        ...
        try:
            ...
            QMessageBox.critical(self, "Decryption Failed", f"Could not perform action.\nError: {e}")
    ...
    def _load_devices(self):
        try:
            ...
            QMessageBox.critical(self, "Error", f"Failed to load devices file: {e}")
        return []
    ...
    def _save_devices(self):
        try:
            ...
            QMessageBox.critical(self, "Error", f"Failed to save devices file: {e}")
    ...
    def add_device(self):
        ...
        dialog = DeviceDialog(self.cred_manager, parent=self)
        if dialog.exec():
            new_device_data = dialog.get_data()
            if any(d["host"] == new_device_data["host"] for d in self.devices):
                QMessageBox.warning(self, "Duplicate", "A device with this host already exists.")
                return
    ...
    def edit_device(self):
        ...
        if not self.cred_manager.get_master_password(self): return
        dialog = DeviceDialog(self.cred_manager, device_data, self)
        if dialog.exec():
    ...
    def backup_selected_device(self):
        async def do_backup(device_info, row_index):
            ...
            if success:
                ...
                QMessageBox.information(self, "Backup Complete", f"Configuration for {device_info['host']} saved to:\n{backup_path}")
            else:
                QMessageBox.critical(self, "Backup Failed", f"Could not back up {device_info['host']}.\n\nError: {output}")
    ...
    def check_for_changes(self):
        async def do_diff(device_info, row_index):
            ...
            if not device_backup_dir.is_dir() or not any(device_backup_dir.iterdir()):
                QMessageBox.information(self, "No Backups", "No backups exist for this device. Please create one first.")
                return
            ...
            progress.close()
            if not success:
                QMessageBox.critical(self, "Error", f"Could not fetch live config: {live_config}")
                return
            with open(latest_backup_path, 'r') as f: backup_config = f.read()
            if backup_config == live_config:
                QMessageBox.information(self, "No Changes", "Live configuration matches the latest backup.")
            else:
