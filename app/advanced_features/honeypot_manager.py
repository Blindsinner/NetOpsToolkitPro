# app/advanced_features/honeypot_manager.py
...
class HoneypotManagerWidget(BaseToolWidget):
    ...
    def shutdown(self):
        """Gracefully stop all running honeypot listeners."""
    ...
    def stop_honeypot(self):
        ...
        if not selected_items:
            self.show_error("Please select a listener to stop.")
            return
        
        row = selected_items[0].row()
        port_item = self.listener_table.item(row, 0)
        if not port_item: return

        port_to_stop = int(port_item.text())
        self.engine.stop_listener(port_to_stop)
        
        for r in range(self.listener_table.rowCount()):
            if self.listener_table.item(r, 0) and self.listener_table.item(r, 0).text() == str(port_to_stop):
                self.listener_table.removeRow(r)
                break
