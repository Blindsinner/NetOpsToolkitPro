...
from PySide6.QtWidgets import (
    QDialog, QVBoxLayout, QGridLayout, QComboBox, QPushButton, QLineEdit,
...
# --- FIX: Renamed class back to its original name ---
class PerformanceMonitoringDialog(QDialog):
    def __init__(self, task_manager: TaskManager, parent=None):
        ...
        self.setWindowTitle("Network Performance Monitoring")
    ...
    def _get_selected_device_info(self):
        ...
        if device_info:
            ...
            # --- FIX: Convert SNMP version to integer ---
    ...
    async def fetch_interfaces(self, device_info):
        ...
        if success and interfaces:
            # Sort interfaces naturally if possible, otherwise alphabetically
            try:
                import natsort
    ...
    def start_monitoring(self):
        ...
        self.update_interface_stats() # Call once immediately
    ...
    async def get_and_display_iface_stats(self, device_info, iface_index):
        ...
        if self.last_stats and self.last_timestamp > 0:
            ...
            if delta_time > 0:
                # Handle counter rollovers (for 32-bit counters)
                ...
                in_bps = (in_oct_diff * 8) / delta_time  # bits per second
                out_bps = (out_oct_diff * 8) / delta_time # bits per second
                in_pps = in_pkt_diff / delta_time       # packets per second
                out_pps = out_pkt_diff / delta_time      # packets per second
