...
from PySide6.QtWidgets import (
    ...
    QDoubleSpinBox, QSpinBox, QComboBox, QWidget
)
...
class AIConfigDialog(QDialog):
    ...
    def load_settings(self):
        ...
        self.on_provider_change(provider) # Trigger this to set defaults and visibility
...
class AIAssistantDialog(QDialog):
    ...
    def run_analysis(self):
        ...
        if not api_key and provider not in ["Ollama"]:
            QMessageBox.warning(self, "API Key Missing", f"Please configure the AI Assistant with your {provider} API key first.")
            return

        # --- FIX: The dictionary comprehension key was wrong ---
        # It should be s.split('/')[-1] to get the last part of the key
        ...
        if not prompt or not context:
            QMessageBox.warning(self, "Input Error", "Please provide both a context and a prompt.")
            return
    ...
    async def get_and_stream_response(self, settings, prompt, context):
        full_response = ""
        # The assistant is now initialized with all settings at once
        async for chunk in self.assistant.get_analysis_stream(settings, prompt, context):
            ...
            # Ensure the cursor is at the end to show the streaming text
